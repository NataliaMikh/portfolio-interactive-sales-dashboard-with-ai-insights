...
def main():
    ...
    with tabs[1]:
        if 'data_uploaded' in st.session_state and st.session_state.data_uploaded:
            ...
            # Apply data cleaning functions only if df exists
        ...
        df['Sales USD'] = df.apply(calculate_sales_total_amount_usd, axis=1)
        ...
        df['Year'] = pd.to_datetime(df['Shipped Date']).dt.strftime('%Y')
        ...
        with st.container():
            ...
            with row1_col3:
                ...
                filtered_df_numeric = filtered_df[['Site', 'Sales USD']].copy()
            ...
            with row2_col1:
                ...
                """, unsafe_allow_html=True)
                st.dataframe(market_segment_sales_profit_summary(filtered_df))
